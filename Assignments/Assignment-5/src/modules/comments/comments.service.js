...
export const createComments = async (inputs) => {
    ...
    const comments = Array.isArray(inputs) ? inputs : inputs.comments;
    ...
    const comment = await Comment.bulkCreate(comments, { validate: true });
...
export const updateCommentById = async (inputs) => {
    ...
    if (!found) {
        ...
        throw new Error("user not authorized", { cause: { status: 403 } });
...
export const findCommentsHaveSpecificWord = async (inputs) => {
    ...
    return { count: result.count, comments: result.rows };
};
